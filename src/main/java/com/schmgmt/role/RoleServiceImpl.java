...
public class RoleServiceImpl implements RoleService {
    ...
    public List<RoleResponse> getActiveRoles() {
        ...
        return roleRepository.findAllByIsActiveTrueOrderByNameAsc()
                .stream()
                .map(this::mapToResponse)
                .toList();
    }
    ...
    public RoleResponse getRoleById(Long primaryKey) {
        ...
        Role role = roleRepository.findById(primaryKey)
                .orElseThrow(() ->
                        new ResourceNotFoundException("Role not found with id: " + primaryKey)
                );
    ...
    public RoleResponse updateRole(
        ...
        if (roleRepository.existsByNameIgnoreCaseAndPrimaryKeyNot(
                roleName,
                primaryKey
        )) {
        ...
        Role role = roleRepository.findById(primaryKey)
                .orElseThrow(() ->
                        new IllegalArgumentException(
                                "Role not found with id: " + primaryKey
                        )
                );
